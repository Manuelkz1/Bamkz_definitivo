...
export function PaymentStatus() {
      ...
      if (status === 'approved' && orderId) {
        ...
        try {
          ...
          if (orderError || !orderData) {
            ...
            // 2. Update order status to confirmed since payment was successful
            console.log('[PaymentStatus] Updating order status to confirmed');
            ...
            if (updateError) {
              console.error('[PaymentStatus] Error updating order status:', updateError.message);
            } else {
              console.log('[PaymentStatus] Order status updated successfully to processing/paid');
            }
  ...
  // Cleanup para pagos fallidos
  useEffect(() => {
    const handleFailedPayment = async () => {
      if ((status === 'rejected' || status === 'failure') && orderId) {
        console.log('[PaymentStatus] Payment failed, cleaning up order:', orderId);
        try {
          // Opcional: Eliminar la orden fallida después de un tiempo
          // Por ahora solo lo logueamos para auditoría
          console.log('[PaymentStatus] Order remains in payment_pending state for audit');
        } catch (e: any) {
          console.error('[PaymentStatus] Error during failed payment cleanup:', e.message);
        }
      }
    };
